...
elif args.language == 'language_X': # Replace 'language_X' with the specific language you want to use.
    train_set = Dataset(os.path.join(args.data_path, 'lan_x/train_data'), tokenizer=tokenizer, sequence_len=sequence_len,
                        token_style=token_style, is_train=True, augment_rate=ar, augment_type=aug_type)
    val_set = Dataset(os.path.join(args.data_path, 'lan_x/dev_data'), tokenizer=tokenizer, sequence_len=sequence_len,
                      token_style=token_style, is_train=False)
    test_set_ref = Dataset(os.path.join(args.data_path, 'lan_x/test_data'), tokenizer=tokenizer, sequence_len=sequence_len,
                           token_style=token_style, is_train=False)
    test_set_asr = Dataset(os.path.join(args.data_path, 'lan_x/test-asr_data'), tokenizer=tokenizer, sequence_len=sequence_len,
                           token_style=token_style, is_train=False)
    test_set = [val_set, test_set_ref, test_set_asr]
elif args.language == 'language_X-bangla':
    train_set = Dataset([os.path.join(args.data_path, 'lan_x/train_data'), os.path.join(args.data_path, 'bn/train_bn')],
                        tokenizer=tokenizer, sequence_len=sequence_len, token_style=token_style, is_train=True,
                        augment_rate=ar, augment_type=aug_type)
    val_set = Dataset([os.path.join(args.data_path, 'lan_x/dev_data'), os.path.join(args.data_path, 'bn/dev_bn')],
